...
6. **Build output**
   ...
   - For production, choose ONE of two paths:
     ...
     **Path B — one-click build (reproducible / batch).** Compose an `outline.json` and run the generator.
       ...
       g. **Use ripple material selectively**: on sparse `cover`, `section-divider`, `stat-highlight`, `big-quote`, or `closing` pages, an optional `surface:{"kind":"ripple",...}` can add `rings`, `flow`, or `caustic` texture. Keep dense reading pages calm, store the seed in the outline, and follow `references/ripple-textures.md`.
