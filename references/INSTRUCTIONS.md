...
>
> Platform adapter files (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`,
> `.cursor/rules/*.mdc`, and the WorkBuddy `SKILL.md`) are thin pointers that all
> say the same thing: *read this file and run `scripts/build.py`.* Edit only
...
- `references/three-3d.md` — **when/what/where to add Three.js 3D scenes**: per-page decision rule (only "atmosphere / key-concept" slides benefit), scene presets (`field` / `nebula` / `object` / `network` / `petals` / `orbs` / `waves`), placement behind content, single-shared-canvas mechanics, and style consistency. Read this whenever a slide could benefit from 3D depth.
...
**Layout snippet library** (`templates/single-page/*.html`): 15 drop-in
...
`chart`. Each ships with demo data and `{{field}}` placeholders; compose a deck
...
- 3D / Three.js: **proactively add live 3D backgrounds only where they strengthen the theme or visual impact** — `field` (floating bokeh), `nebula` (denser particles), `object` (rotating wireframe icosahedron), `network` (nodes + links), `petals`, `orbs`, `waves`. Best hosts: `cover`, `section-divider`, `stat-highlight`, `closing`; never on text-dense reading slides. One shared WebGL canvas swaps scenes by visible slide; the library inlines automatically only when a `three` field exists; style is auto-tuned to the light liquid-glass theme and degrades to a static gradient if WebGL is unavailable (see `references/three-3d.md`). **Also fully automatic for the user: the operator decides which pages, picks the scene type, writes the `three` fields, and builds — the user never writes code or fields.**
- 3D scenes: **proactively add Three.js 3D only on "atmosphere / key-concept" slides** — cover, section dividers, a key concept page, closing. Use `field` (drifting particles), `nebula` (denser swirl), `object` (translucent glass solid, offset right), `petals`, `orbs`, or `waves`. Never on pure-info slides (bullets/grid-cards/timeline/comparison/two-column/chart); never on every slide. All scenes render BEHIND the text via one shared canvas and auto-inherit the iOS pastel palette. See `references/three-3d.md`. **Also fully automatic for the user: the operator scans, picks the scene, writes the `three` field, and builds — zero user code.**
